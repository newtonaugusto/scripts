...
def main():
    ...
    if st.sidebar.checkbox("Entrar"):
        ...
        if result:               
            ...
            final = date.today().replace(day=1) - timedelta(days=1) 
            initial = final - relativedelta(months=1) + timedelta(days=1)
            ...
            if task == "Dashboard":
                ...
                if filtered_df.empty:
                    ...
                    tmp = filtered_df['business_name'].map(horas_cliente)
                    horas_cliente_df = []
                    for row_index, row in enumerate(horas_cliente.axes[0]):
                        horas_cliente_df.append([row, horas_cliente[row_index]])

                    horas_cliente_df = pd.DataFrame(np.array(horas_cliente_df), columns = ['business_name', 'horas'])
                    ...
                    st.subheader("**Total de horas:**")
                    st.subheader(f'**{total_hours.days*24 + (total_hours.seconds//3600)} h {(total_hours.seconds//60)%60} min**')
                ...
                possible_months = []
                current_date_filtered_df = start_date
                while current_date_filtered_df <= end_date: 
                ...
                selected_month_to_report = st.selectbox("Selecione o mês de referênca", possible_months)
                ...
                selected_month_to_report_end_day = selected_month_to_report + relativedelta(months=1) - timedelta(days=1)
                ...
                try:
                    ...
                    link_mensal_report= f'<a href="data:file/csv;base64,{b64_mensal_report}" download="relatorio-mensal-{selected_month_to_report}.csv">Download csv file</a>'
                ...
                try:
                    ...
                    linkou_mensal_report= f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64a_mensal_report}" download="relatorio-mensal-{selected_month_to_report}.xlsx">Download excel file</a>'
