...
def generate_json(initial_date_request, final_date_request):
...
def update_report(s3, buffer_report):
    
    inicio = time()
    
    initial_date = datetime(2020, 11, 1)
    final_date = datetime(2021, 12, 1)

    dayli_buffer = io.BytesIO()
    # dayli_buffer.write(json.dumps(Request(date_request).request_tickets()).encode())
    dayli_buffer.write(json.dumps(generate_json(initial_date_request=initial_date, final_date_request=final_date)).encode())
    dayli_buffer.seek(0)
    ...
    dayli_buffer.seek(0)
    df1 = pd.read_csv(buffer_report, delimiter=";")
    buffer_report.close()
