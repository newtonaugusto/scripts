...
# 	menu = ["Home", "Login", "Cadastrar"]
	...
	if choice == "Home":
		st.subheader("Home")
		st.text("Movidash é uma aplicação desenvolvida\npara prover indicadores gráficos\nao corpo diretivo da INFOMACH.\n\nApp desenvolvido em abr/2021 pela Equipe Dev/Cloud.")
		...
		if st.sidebar.checkbox("Entrar"):
			...
			if result:
				...
				task = st.selectbox("Tarefas", ["Dashboards", "Perfis"])
				if task == "Dashboards":
					# st.subheader("PAINEL MOVIDASH")
					# link = '[MOVIDASH](http://localhost:8501/)'
					...
					st.subheader("Perfis dos usuários cadastrados")
		...
		if st.button("Cadastrar"):
			...
			st.info("Acesse o Login para entrar na página")
